...
prepare_model_par <- function(input.file,
  ...
  if (sigma==0){
    ...
    for(x in (0:(l - 1))){
      ...
      # JS.d <- foreach(si = 0:(sigma-1)
      JS.d <- foreach(si = 0:(sigma-1),
                      .combine = rbind,
                      .packages = c("reshape2")) %dopar%
        melt.one.sigma(si=si)
      JS.melt <- rbind(JS.melt, JS.d)
    }
    stopCluster(cl)
  }
  JS.melt$phase <- phase
